...
int subprocess_create_named_pipe_helper(void **rd, void **wr) {
	...
	const void *const invalidHandleValue = SUBPROCESS_PTR_CAST(
		void *, ~(SUBPROCESS_CAST(subprocess_intptr_t, 0)));
...
#pragma warning(pop)
	...
	*rd = CreateNamedPipeA(
		name, pipeAccessInbound | fileFlagOverlapped,
...
int subprocess_create_ex(const char *const commandLine[], int options,
	...
	if (subprocess_option_no_window ==
	...
	if (subprocess_option_inherit_environment !=
		...
		if (SUBPROCESS_NULL == environment) {
			used_environment =
				SUBPROCESS_CONST_CAST(char *, "\0\0");
		} else {
			...
			used_environment =
				SUBPROCESS_CAST(char *, _alloca(len));
	...
	if (options & subprocess_option_enable_async) {
		...
		if (!CreatePipe(&rd, &wr,
				SUBPROCESS_PTR_CAST(LPSECURITY_ATTRIBUTES,
						    &saAttr),
				0)) {
			return -1;
		}
	...
	if (subprocess_option_combined_stdout_stderr ==
		...
		fd = _open_osfhandle(
			SUBPROCESS_PTR_CAST(subprocess_intptr_t, rd), 0);
	...
	if (options & subprocess_option_enable_async) {
		out_process->hEventOutput = CreateEventA(
			SUBPROCESS_PTR_CAST(LPSECURITY_ATTRIBUTES, &saAttr), 1,
			1, SUBPROCESS_NULL);
		out_process->hEventError = CreateEventA(
			SUBPROCESS_PTR_CAST(LPSECURITY_ATTRIBUTES, &saAttr), 1,
			1, SUBPROCESS_NULL);
	} else {
	...
	for (i = 0; commandLine[i]; i++) {
		...
		need_quoting = strpbrk(commandLine[i], "\t\v ") !=
			       SUBPROCESS_NULL;
		if (need_quoting) {
			commandLineCombined[len++] = '"';
		}
...
#pragma clang diagnostic pop
	...
	if (0 != posix_spawn_file_actions_adddup2(&actions, stdinfd[0],
						  STDIN_FILENO)) {
		posix_spawn_file_actions_destroy(&actions);
		return -1;
	}
	...
	if (0 != posix_spawn_file_actions_adddup2(&actions, stdoutfd[1],
						  STDOUT_FILENO)) {
		posix_spawn_file_actions_destroy(&actions);
		return -1;
	}
	...
	if (subprocess_option_combined_stdout_stderr ==
	    (options & subprocess_option_combined_stdout_stderr)) {
		if (0 != posix_spawn_file_actions_adddup2(
...
#pragma clang diagnostic ignored "-Wold-style-cast"
	...
	if (subprocess_option_search_user_path ==
		...
		if (0 != posix_spawnp(&child, commandLine[0], &actions,
				      SUBPROCESS_NULL,
		...
		if (0 != posix_spawn(&child, commandLine[0], &actions,
				     SUBPROCESS_NULL,
				     (char *const *)commandLine,
				     used_environment)) {
			posix_spawn_file_actions_destroy(&actions);
...
int subprocess_join(struct subprocess_s *const process,
	...
	if (out_return_code) {
		if (!GetExitCodeProcess(process->hProcess,
					SUBPROCESS_PTR_CAST(unsigned long *,
							    out_return_code))) {
			return -1;
		}
	}
...
unsigned subprocess_read_stdout(struct subprocess_s *const process,
	...
	handle = SUBPROCESS_PTR_CAST(
		void *, _get_osfhandle(_fileno(process->stdout_file)));
	...
	if (!ReadFile(handle, buffer, size, &bytes_read,
		...
		if (error == errorIoPending) {
			if (!GetOverlappedResult(
				    handle,
				    SUBPROCESS_PTR_CAST(LPOVERLAPPED,
							&overlapped),
				    &bytes_read, 1)) {
...
unsigned subprocess_read_stderr(struct subprocess_s *const process,
	...
	handle = SUBPROCESS_PTR_CAST(
		void *, _get_osfhandle(_fileno(process->stderr_file)));
...
int subprocess_alive(struct subprocess_s *const process) {
...
#if defined(_MSC_VER)
	{
		...
		is_alive = wait_object_0 !=
			   WaitForSingleObject(process->hProcess, zero);
	}
#else
	{
		int status;
