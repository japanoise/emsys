...
void stringRectangle(void) {
	...
	if (boty >= buf->numrows) {
		buf->markx = 0;
	} else if (botx > buf->row[boty].size) {
...
void copyRectangle(void) {
	...
	if (E.rx > 0 && E.ry > 0 && E.rx <= INT_MAX / E.ry) {
		E.rectKill = xcalloc((E.rx * E.ry) + 1, 1);
	} else {
		setStatusMessage("Rectangle too large");
		return;
	}
